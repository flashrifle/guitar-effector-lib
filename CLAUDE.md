...
**보류 중: 노이즈 게이트·튜너.** Boss NS-2(threshold/decay/mode)와 ISP Decimator(threshold)는 컨트롤 확인까지 끝냈지만 넣지 않았다. `gate` 카테고리가 없고, 새 계열 신설은 위 표대로 소비 측 사전 합의가 필요하기 때문이다. 합의되면 바로 추가할 수 있다.
