...
## 데이터 출처 원칙이 한 번 넓어졌다 (0.3.0)

0.2.0까지는 **"모델러 세 곳(Helix·QC·Fractal)이 based-on으로 공개한 실물"**만 넣었다. 0.3.0에서 리버브 8종·딜레이 9종을 추가하며 이 원칙을 넓혔다.

**근거는 소비 측의 실제 요청 로그다.** 사용자가 두 번 이상 찾은 장비(Empress Echosystem, Boss DD-3)가 카탈로그에 없어 LLM 폴백으로 빠지고 있었고, `reverb` 카테고리는 아예 0개였다. 모델러가 모델링하지 않았다는 이유로 Strymon·Walrus·Empress를 빼두는 건 라이브러리의 목적(조회 비용 절감)과 어긋난다.

**넓어진 기준**: 모델러의 based-on 목록에 있거나, **실사용 요청 근거가 있는 실물**. 여전히 "유명하니까 넣는다"는 안 된다 — 근거가 있어야 한다.

노브 검증 기준은 그대로다. 17종 전부 공식 매뉴얼·제조사 페이지에서 확인했고, 확인 못 한 건 넣지 않았다.

### 소비 측이 의존하는 것 (어기면 조용히 깨진다)

| 규칙 | 어겼을 때 |
|---|---|
| `category`는 정해진 값만 | 모르는 값은 소비 측에서 `other`로 떨어져 아이콘이 무채색이 된다. 새 계열을 만들려면 **미리 알릴 것** — 소비 측 매핑이 같은 릴리스에 들어가야 한다 |

**보류 중: 노이즈 게이트.** Boss NS-2(threshold/decay/mode)와 ISP Decimator(threshold)는 컨트롤 확인까지 끝냈지만 넣지 않았다. `gate` 카테고리가 없고, 새 계열 신설은 소비 측 사전 합의가 필요하기 때문이다. 합의되면 바로 추가할 수 있다. 튜너도 같은 상태다.
| `parameter`를 비우지 않기 | 빈 배열은 검색·조회 양쪽에서 통째로 안 보인다. 노브를 모르면 **항목 자체를 넣지 말 것** (캐비닛 100종이 의도적으로 그 상태) |
| `parameter`는 camelCase | 이 값이 사용자 화면에 그대로 나간다 |
| 흔한 단어를 단독 모델명으로 쓰지 않기 | 모델명만으로 찾는 지름길이 죽는다. 지금 `flanger`(MXR/A-DA)와 `rocket`(Trainwreck/Paul Ruby)이 그 상태다 |
| 기존 `company`/`model` 문자열 불변 | 소비 측 캐시 키와 테스트가 정확한 문자열에 걸려 있다. 개명은 major이고 **사전 통보가 필요하다** |
